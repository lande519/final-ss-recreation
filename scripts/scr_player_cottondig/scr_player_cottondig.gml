    ...
    if (!place_meeting(x, y, obj_cottonsolid))
    {
        if (sprite_index == spr_cottonDashIntro)
        {
            if (sprite_animation_end())
                sprite_index = drill_sprites[argument0];
        }
        else
        {
            sprite_index = drill_sprites[argument0];
...
{
    var target_xscale, angle_diff, rep, dt, _ledge, check_ledge_slide;
    ...
    target_xscale = sign(lengthdir_x(1, cottonDirection));
    ...
    if (!savedCottonSolid && scr_solid(x, y + sign(vsp), false) && !place_meeting(x, y + sign(vsp), obj_destructibles))
    {
        ...
        check_ledge_slide = sprite_index != spr_player_PZ_werecotton_drill_ddown || sign(xscale) == sign(_ledge);

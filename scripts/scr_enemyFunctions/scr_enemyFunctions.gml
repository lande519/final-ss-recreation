...
function scr_scareenemy()
{
    ...
    if (!jumpedFromBlock && scr_enemy_playerisnear(400, 130, undefined, 90, player_object) && collision_line(x, y, player_object.x, player_object.y, obj_solid, false, true) == -4 && (player_object.state == States.mach3 || player_object.state == States.ratmount && (player_object.sprite_index = spr_player_ratmountdashjump or player_object.sprite_index = spr_player_ratmountmach3)|| ((player_object.state == States.doughmount && object_index != obj_fancypancake) && abs(movespeed) >= 12) || player_object.state == States.doughmountspin || (player_object.movespeed >= 10 && player_object.state == States.minecart)))
    {

...
function cutscene_judgment_start()
{
    ...
    with (obj_judgmentpainter)
    {
        if (!active)
        {
            ...
            d = array_clone(givenJudgment.properties.dialog);
            ...
            for (i = 0; i < array_length(d); i++)
            {
                s = scribble(d[i]).wrap(sprite_get_width(spr_dialobox_temp) - 80).line_spacing(30).starting_format(font_get_sprite(global.npcfont, true)).align(0, 0);

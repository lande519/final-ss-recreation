var surf_mask, dp, pg, l, p, yy, ypad, xx, tpad, goff, cy;
...
surf_mask = surface_create(tvwidth, tvheight);
...
dp = buffering ? targetPage : currentPage;
pg = getPage(dp);
pg.draw();
l = pg.getContent(0);
p = struct_get(l, "properties");
...
goff = round(sin(current_time / 1000) / 1.9);
cy = (scanline.y > 0) ? scanline.y : (tvheight - 85);

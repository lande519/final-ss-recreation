var player, saveroom_ind, gems_refined, collect_amount;

if (!instance_exists(obj_minesgem))
    instance_create(x, y, obj_minesgem);

player = other.id;
saveroom_ind = ds_list_find_index(global.SaveRoom, id);
gems_refined = ds_list_find_value(global.SaveRoom, saveroom_ind + 1);
...
if (player.minesGemCount > 0 && gemsRefined < 4)
{
    ...
    collect_amount = 50 * player.minesGemCount;

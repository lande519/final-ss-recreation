var painter_sprites, _end_of_text, dir;

if (!active)
    exit;

input_advance = input_check_pressed("jump") || input_check_pressed("up");
painter_sprites = ds_map_find_value(painterExpressionMap, painterMood);
...
switch (progression)
{
    ...
    case 1:
        ...
        targetpos.y = ((camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0])) - sprite_height) + 4;
        _end_of_text = typist.get_state() >= 1;
...
if (progression == 0)
{
    dir = point_direction(x, y, targetpos.x, targetpos.y);

var cam_width, cam_height, cam_x, cam_y, c, i, set;
...
if (sucrose_lighting)
{
    ...
    if (sucrose_lighting && surface_exists(sucroseSurface))
    {
        ...
        for (i = 0; i < array_length(sucrose_color_array); i++)
        {
        ...
        with (obj_parent_player)
        {
            if (visible && bbox_in_camera(id, c, 32))
            {
                ...
                if (state == States.charge && image_index < 5)
                {
        ...
        {
            set = abs((image_index / image_number) - 1);

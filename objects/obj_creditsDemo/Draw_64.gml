...
if (creditPhase == 1)
{
    if (currentCategory >= 0 && currentCategory < categoryCount)
    {
        ...
        {
            _nameArr = creditEntries[i][1];
            array_foreach(_nameArr, function(argument0, argument1)
            {
                argument0.draw();
            });
        }
        ...
        draw_text_scribble(camera_get_view_width(view_camera[0]) / 2, 100, string("[fa_center][fa_middle][spr_smallfont]{0}", string_upper(creditEntries[currentCategory][0])));
        draw_set_color(c_white);
    }
}

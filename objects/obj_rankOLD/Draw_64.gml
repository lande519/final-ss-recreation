var i, _string, _string_length, _xx, _yyoffset, _janx, pal, _yy;
...
i = 0;
_string = round(string(global.HighestCombo));
...
{
    _xx = 850 + ((30 * i) - (string_width(_string) / 2));
    _yyoffset = ((i % 2) == 0) ? -4 : 0;
...
for (i = 0; i < _string_length; i++)
{
    _xx = ((pshake ? irandom_range(-4, 4) : 0) + 750 + ((string_width(_string) / _string_length) * i)) - (string_width(_string) / 2);
    _yy = (pshake ? irandom_range(-4, 4) : 0) + 385;
    pal = colors[i];
    _yyoffset = ((i % 2) == 0) ? -4 : 0;

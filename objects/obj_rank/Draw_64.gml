var i, _text_scribble, _cx, _cy, _dx, _dy, c, _xx, _yy, s, _shk, _ind, _janx, _string, _string_length, pal, _yyoffset;
...
i = 0;
_text_scribble = "[c_red][fa_middle][fa_center][spr_font_dialogue]";
...
for (c = 0; c < array_length(confecti_sprites); c++)
{
    _xx = 555 + (75 * c);
    _yy = 230 + clipboardY;
...
for (s = 0; s < array_length(secrets_collected_visual); s++)
{
    _xx = 650 + (75 * s);
    _yy = 325 + clipboardY;
...
i = 0;
_string = string(round(score_total_visual));
...
for (i = 0; i < _string_length; i++)
{
    _xx = (750 + ((string_width(_string) / _string_length) * i)) - (string_width(_string) / 2);
    _yy = 385;
    pal = score_total_colors[i];
    _yyoffset = ((i % 2) == 0) ? -4 : 0;

var reserved_gates, available_critics, i, lv, found_rank, si, xo, yo;

reserved_gates = [];
available_critics = [spr_paintCritic_1, spr_paintCritic_2, spr_paintCritic_3, spr_paintCritic_4];

with (obj_paintcritic)
{
    i = array_get_index(available_critics, sprite_index);
...
found_rank = string_lower(ini_read_string("Ranks", lv, ""));
ini_close();

if (found_rank == "" || instance_exists(obj_tutorialBlock))
    instance_destroy();

si = irandom_range(0, array_length(available_critics) - 1);
...
xo = (sprite_get_width(spr_npcbubble) / 2) + 16;
yo = (sprite_get_height(spr_npcbubble) / 2) + 16;

var _channels, _coreflags, _studioflags, settings_struct, path, plat, banks, i, b, bank;

_channels = 256;
_coreflags = FMOD_INIT.NORMAL;
_studioflags = FMOD_STUDIO_INIT.NORMAL;
global.FMODSTUDIOSYSTEM = fmod_studio_system_create();
show_debug_message("fmod_studio_system_create: " + string(fmod_last_result()));
settings_struct = new FmodSystemAdvancedSettings();
...
for (i = 0; i < array_length(banks); i++)
{
    b = working_directory + path + plat + "/" + banks[i];
    bank = fmod_studio_system_load_bank_file(b, FMOD_STUDIO_LOAD_BANK.NORMAL);
...
updateFFT = function()
{
    var channel_group, size, required_size, data_size, data_channels, i, z;
    
    if (!initializedDSP)
    {
        channel_group = fmod_studio_bus_get_channel_group(musicBus);
    ...
    required_size = fmod_dsp_get_parameter_data(global.FMOD_DSP_FFT, FMOD_DSP_FFT.SPECTRUMDATA, global.FMOD_DSP_FFTBuffer);
    
    if (buffer_get_size(global.FMOD_DSP_FFTBuffer) < required_size)
        buffer_resize(global.FMOD_DSP_FFTBuffer, required_size);
    
    data_size = buffer_read(global.FMOD_DSP_FFTBuffer, buffer_s32) / 2;
    data_channels = buffer_read(global.FMOD_DSP_FFTBuffer, buffer_s32);
